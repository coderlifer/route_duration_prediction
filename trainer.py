# Remove commented clause in trainer_1.py
...
from hparams import build_from_set
...
def train(name, hparams, multi_gpu=False, n_models=1, train_completeness_threshold=0.01,
          ...
          side_split=True, max_steps=None, save_from_step=None, do_eval=True, predict_window=1, is_train=True):
    ...
    train_size = 1639424
    eval_size = 409856
    real_train_pages = train_size
    real_eval_pages = eval_size
    ...
    with tf.Session(config=tf.ConfigProto(allow_soft_placement=True,
        ...
        for epoch in range(max_epoch):
            ...
            if trainer.has_active():
                status += ", frwd/side best MAE=%.3f/%.3f, SMAPE=%.3f/%.3f; avg MAE=%.3f/%.3f, SMAPE=%.3f/%.3f," \
                          " %d am" % \
                          (eval_mae.best_epoch, eval_mae_side.best_epoch, eval_smape.best_epoch,
                           eval_smape_side.best_epoch,
...
def predict(checkpoints, hparams, return_x=False, verbose=False, predict_window=6, back_offset=0, n_models=1,
    ...
    with tf.variable_scope('input'):
        with tf.device("/cpu:0"):
            ...
            pipe = InputPipe(inp=None, features='./data/val_normed.csv', n_pages=409856, mode=ModelMode.PREDICT,
    ...
    if n_models == 1:
        ...
        for i in range(n_models):
            ...
            with tf.variable_scope(prefix):
                models.append(
                    Model(pipe, hparams, is_train=False, seed=seed, asgd_decay=asgd_decay, graph_prefix=prefix))
    ...
    predictions.to_csv('data/cpt/s32/output.csv', index=False, float_format='%.3f')
...
if __name__ == '__main__':
    ...
    parser.add_argument('--hparam_set', default='s32',
                        help="Hyperparameters set to use (see hparams.py for available sets)")
    parser.add_argument('--n_models', default=1, type=int, help="Jointly train n models with different seeds")
    parser.add_argument('--multi_gpu', default=False, action='store_true',
                        help="Use multiple GPUs for multi-model training, one GPU per model")
    ...
    parser.add_argument('--patience', type=int, default=2,
                        help="Early stopping: stop after N epochs without improvement. Requires do_eval=True")
    ...
    parser.add_argument('--eval_memsize', type=int, default=5,
                        help="Approximate amount of avalable memory on GPU, used for calculation of optimal "
    ...
    parser.add_argument('--gpu_allow_growth', default=False, action='store_true',
                        help='Allow to gradually increase GPU memory usage instead of grabbing all available '
                             'memory at start')
    parser.add_argument('--save_best_model', default=False, action='store_true',
                        help='Save best model during training. Requires do_eval=True')
    parser.add_argument('--no_forward_split', default=True, dest='forward_split', action='store_false',
                        help='Use walk-forward split for model evaluation. Requires do_eval=True')
    parser.add_argument('--side_split', default=False, action='store_true',
                        help='Use side split for model evaluation. Requires do_eval=True')
    parser.add_argument('--no_eval', default=True, dest='do_eval', action='store_false',
                        help="Don't evaluate model quality during training")
    parser.add_argument('--no_summaries', default=True, dest='write_summaries', action='store_false',
                        help="Don't Write Tensorflow summaries")
    parser.add_argument('--verbose', default=False, action='store_true',
                        help='Print additional information during graph construction')
    parser.add_argument('--asgd_decay', type=float, help="EMA decay for averaged SGD. Not use ASGD if not set")
    parser.add_argument('--no_tqdm', default=True, dest='tqdm', action='store_false',
                        help="Don't use tqdm for status display during training")
    parser.add_argument('--max_steps', type=int, help="Stop training after max steps")
    parser.add_argument('--save_from_step', type=int,
                        help="Save model on each evaluation (10 evals per epoch), starting from this step")
    parser.add_argument('--predict_window', default=63, type=int, help="Number of days to predict")
    parser.add_argument('--is_train', default=1, type=int, help="Number of days to predict")
    ...
    if args.is_train:
        ...
        preds = predict(['data/cpt/s32/cpt-10880'], param_dict['hparams'], verbose=True, back_offset=60,
                        n_models=3)
        print('\nOutput:')
        print(preds.shape)
        print(preds)
    ...
    # result = train("definc_attn", hparams, n_models=1, train_sampling=1.0, eval_sampling=1.0, patience=5,
    # multi_gpu=True,
    #                save_best_model=False, gpu=0, eval_memsize=15, seed=5, verbose=True, forward_split=False,
    #                write_summaries=True, side_split=True, do_eval=False, predict_window=63,
    #                asgd_decay=None, max_steps=11500,
