...
def default_init(seed):
    ...
    return layers.variance_scaling_initializer(factor=1.0, mode="FAN_AVG", uniform=True, seed=seed)
...
#
#     # [width(attn_window), channels(batch), n_heads] ->
#     # [height(1), width(attn_window), channels(batch), multiplier(n_heads)]
#     attn_filter = attns_max[tf.newaxis, :, :, :]
#     # [batch(readout_depth), height=1, width=n_days, channels=batch] ->
#     # [batch(readout_depth), height=1, width=predict_window, channels=batch*n_heads]
#     averaged = tf.nn.depthwise_conv2d_native(inp, attn_filter, [1, 1, 1, 1], 'VALID')
#     # [batch, height=1, width=predict_window, channels=readout_depth*n_neads] ->
#     # [batch(depth), predict_window, batch*n_heads]
...
class Model:
    def __init__(self, inputPipe: InputPipe, hparams, is_train, seed, graph_prefix=None,
                 asgd_decay=None, loss_mask=None):
        """
        Encoder-decoder prediction model
        :param inputPipe: Input tensors
        :param hparams:
        :param is_train:
        :param seed:
        ...
        self.train_window = 101
        # self.attn_window = 500
        # self.predict_window = 500
        self.mode = ModelMode.TRAIN

        encoder_output, h_state, c_state = make_encoder(
            inputPipe.time_x, self.encoder_features_depth, is_train, hparams, seed, transpose_output=False)
        # Encoder activation losses
        enc_stab_loss = rnn_stability_loss(encoder_output, hparams.encoder_stability_loss / self.train_window)
        enc_activation_loss = rnn_activation_loss(encoder_output, hparams.encoder_activation_loss / self.train_window)

        # Convert state from cuDNN representation to TF RNNCell-compatible representation
        encoder_state = convert_cudnn_state_v2(
            h_state, hparams, c_state, dropout=hparams.gate_dropout if is_train else 1.0)

        # # Attention calculations
        # # Compress encoder outputs
        # enc_readout = compressed_readout(
        #     encoder_output, hparams, dropout=hparams.encoder_readout_dropout if is_train else 1.0, seed=seed)
        ...
        # attn_features, attn_weights = attn_readout_v3(
        #     enc_readout, self.attn_window, hparams.attention_heads, fingerprint, seed=seed)
        attn_features = None

        # Run decoder
        decoder_targets, decoder_outputs = self.decoder(
            encoder_state, attn_features if hparams.use_attn else None, inputPipe.time_y, inputPipe.true_x[:, -1])
        # Decoder activation losses
        dec_stab_loss = rnn_stability_loss(decoder_outputs, hparams.decoder_stability_loss / self.predict_window)
        dec_activation_loss = rnn_activation_loss(
            decoder_outputs, hparams.decoder_activation_loss / self.predict_window)

        # Get final denormalized predictions
        self.predictions = decode_predictions(decoder_targets, inputPipe)

        # Calculate losses and build training op
        if self.mode == ModelMode.PREDICT:
            ...
            self.mae, smape_loss, self.smape, self.loss_item_count = calc_loss(
                self.predictions, inputPipe.true_y, additional_mask=loss_mask)
            if is_train:
                # Sum all losses
    ...
    def decoder(self, encoder_state, attn_features, prediction_inputs, previous_y):
        ...
        predict_days = self.predict_window
        # print('\nprediction_inputs: {}'.format(prediction_inputs.shape.as_list()))  # [256, 1, 6]
        # print('predict_days: {}\n'.format(predict_days))
        assert prediction_inputs.shape[1] == predict_days

        # [batch_size, time, input_depth] -> [time, batch_size, input_depth]
        inputs_by_time = tf.transpose(prediction_inputs, [1, 0, 2])  # [1, 256, 6]
        # print('\ninputs_by_time: {}'.format(inputs_by_time.shape.as_list()))
        ...
        def loop_fn(time, prev_output, prev_state, array_targets: tf.TensorArray, array_outputs: tf.TensorArray):
            ...
            # [1, batch_size, input_depth]
            features = inputs_by_time[time]  # [256, 6]
            ...
            if attn_features is not None:
                ...
                # print('\nprev_output: {}'.format(prev_output.shape.as_list()))  # [256, 6, 1]
